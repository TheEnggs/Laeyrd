...
export default class ExtensionController {
  ...
  private async loadFromFile() {
    try {
      const filePath = vscode.Uri.joinPath(this.context.globalStorageUri, this.fileName);
      const bytes = await vscode.workspace.fs.readFile(filePath);
      const text = new TextDecoder("utf8").decode(bytes);
      const parsed = JSON.parse(text);
      this.extensionConfiguration = { ...this.extensionConfiguration, ...parsed };
    } catch (err) {
  ...
  private async writeFile() {
    const filePath = vscode.Uri.joinPath(this.context.globalStorageUri, this.fileName);
    const data = new TextEncoder().encode(JSON.stringify(this.extensionConfiguration, null, 2));
    await vscode.workspace.fs.writeFile(filePath, data);
  }

  async setValue<T extends keyof ExtensionConfiguration>(key: T, value: ExtensionConfiguration[T]) {
