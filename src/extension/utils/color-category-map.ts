import { ColorCategory, ColorTab, ThemeColor } from "../../types/theme";

const colorCategoryMap: Record<
  string,
...
export function transformColorsToColorTabs(
  ...
  Object.entries(groupedColors).forEach(([category, categoryColors]) => {
    if (!categoryColors || typeof categoryColors !== "object") {
      console.warn(
        "Invalid categoryColors for category:",
        category,
        categoryColors
      );
      return;
  ...
  const tabStructure: Record<
    string,
    { name: string; icon: string; categoryMapping: Record<string, string> }
  > = {
    base: {
      name: "Base Colors",
      icon: "Palette",
  ...
  const tabGroups: Record<string, Record<string, ThemeColor[]>> = {};
  ...
  Object.entries(colors).forEach(([colorKey, colorValue]) => {
    try {
      ...
      if (!colorInfo || typeof colorInfo !== "object") {
        ...
        if (!tabGroups[targetTab]) tabGroups[targetTab] = {};
  ...
  const colorTabs: ColorTab[] = Object.entries(tabStructure)
    .map(([tabId, tabConfig]) => {
      const categories: ColorCategory[] = Object.entries(
        tabConfig.categoryMapping
      )
